...
/** Get the HTML to display flash messages
 * @return string
 */
function ThemeFlashMessages($trunc = true){
	...
	$data = array();
	foreach ($_SESSION['flashmsg'] as $severity => $messages){
		if (empty($messages)) continue;

		$html = '<ul class="'.$severity.'">';
		foreach ($messages as $msg)
			$html .= '<li>'.htmlentities($msg).'</li>';
		$html .= '</ul>';
...
function ThemeBegin($subtitle, $js_script, $popup_menu, $body_events)
{
  ...
  httpNoCacheHeaders();
	...
	$sere = array(
		'{TITLE}' => LNG_TITLE,
		'{SUBTITLE}' => $subtitle,
		'{SUBTITLE_LINK}' => '?m='.trim(CURRENT_MODULE, '/'),
		...
		'{MAINMENU}' => $mainmenu,
		'{INFO_TITLE}' => LNG_INFO,
		'{INFO_USERNAME_TITLE}' => LNG_INFO_USERNAME,
		'{INFO_USERNAME}' => htmlEntitiesEx($GLOBALS['userData']['name']),
		'{INFO_DATE_TITLE}' => LNG_INFO_DATE,
		'{INFO_DATE}' => htmlEntitiesEx(gmdate(LNG_FORMAT_DATE, CURRENT_TIME)),
		'{INFO_TIME_TITLE}' => LNG_INFO_TIME,
		'{INFO_TIME}' => htmlEntitiesEx(gmdate(LNG_FORMAT_TIME, CURRENT_TIME)),
		'{INFO_TIMEZONE}' => htmlEntitiesEx(date_default_timezone_get()),
		'{LANGUAGE}' => $GLOBALS['userData']['language'],
		'{BODY_PAGE}' => CURRENT_MODULE,
		'{BASE_HREF}' => '//'.$_SERVER['HTTP_HOST'].'/'.(  $_SERVER['SCRIPT_NAME']  ),
		'{FLASHMSG}' => ThemeFlashMessages(),
	);
...
function ThemeEnd()
{
  echo file_get_contents(THEME_PATH.'/footer.html');
	...
	$flashmsg = ThemeFlashMessages();
	if (!empty($flashmsg)){
		$flashmsg_data = json_encode(array('#flashmsg' => $flashmsg));
		echo <<<HTML
		<script>
		window.phpAppend($flashmsg_data);
		</script>
HTML;
...
function themeSmall($subtitle, $data, $js_script, $popup_menu, $body_events)
{
  ...
  httpNoCacheHeaders();
	...
	$sere = array(
		...
		'{BODY_PAGE}' => defined('CURRENT_MODULE')? CURRENT_MODULE : (isset($_GET['botsaction'])? 'botsaction-'.htmlentities($_GET['botsaction']) : 'unknown' ),
		'{FLASHMSG}' => ThemeFlashMessages(),
	);
