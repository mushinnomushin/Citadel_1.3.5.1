...
$filter['nat']     = isset($_GET['nat'])     ? intval($_GET['nat'])     : 0;
$filter['online']  = isset($_GET['online'])  ? intval($_GET['online'])  : 0;
...
else
{
  ...
  while(($mt = @mysql_fetch_row($r)))
  {
    ...
    THEME_LIST_ROW_BEGIN.
      ...
      str_replace(array('{WIDTH}', '{TEXT}'), array('auto', botPopupMenu($mt[0], 'botmenu', $mt[9], $mt[7])),                          $themeText).
